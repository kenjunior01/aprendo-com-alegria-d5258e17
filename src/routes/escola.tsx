...
export const Route = createFileRoute("/escola")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Painel para professores: turmas, métricas dos alunos e exportação CSV.",
      },
    ...
    links: [{ rel: "canonical", href: "https://kidoz.online/escola" }],
  }),
...
function EscolaPage() {
  ...
  const reloadAll = async (
    ...
    setRanking(rk.top);
    setRankingAll(("all" in rk ? rk.all : rk.top) ?? rk.top);
    setTimeline(tl.points);
    setAlerts(al.alerts);
  };

  const reloadDashboard = (cls = selectedClass, subject = subjectFilter, days = daysFilter) =>
    reloadAll(cls, subject, days, studentFilter);
  ...
  const exportCsv = () => {
    ...
    const subjHeaders = subjCols.flatMap((s) => [
      `${SUBJECT_LABELS[s] ?? s} sessões`,
      `${SUBJECT_LABELS[s] ?? s} precisão %`,
      `${SUBJECT_LABELS[s] ?? s} min`,
    ]);
    const header = [
      "Nome",
      "Ano",
  ...
  if (loading) {
    return (
      <div className="min-h-[100dvh] bg-background">
        ...
        <KidLoader compact label="A carregar painel da escola…" />
      </div>
    );
  }
      ...
      <main id="main-content" className="mx-auto max-w-5xl px-4 py-6">
        <header className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Gere turmas, acompanha progresso por disciplina e exporta relatórios.
            </p>
          </div>
        </header>
        ...
        {!isTeacher && (
          <div className="card-chunky mb-6 rounded-2xl border-2 border-border bg-accent/20 p-4">
            <p className="mb-3 text-sm">
              Estás identificado como conta de família. Queres também ativar funcionalidades de
              professor?
            </p>
            <Button
              onClick={async () => {
        ...
        <Tabs defaultValue="schools">
          <TabsList className="mb-4">
            ...
            {selectedClass && (
              <TabsTrigger value="dash">Relatório · {selectedClass.name}</TabsTrigger>
            )}
          </TabsList>
          ...
          <TabsContent value="schools">
            ...
            <div className="mt-4 space-y-4">
                        ...
                        {schoolClasses.map((c) => (
                          <ClassCard
                            ...
                            onDelete={async () => {
                              if (
                                !confirm(
                                  `Apagar turma "${c.name}"? Esta ação não pode ser desfeita.`,
                                )
                              )
                                return;
              ...
              {schools.length === 0 && (
                <p className="text-sm text-muted-foreground">
                  Ainda não tens escolas. Cria a primeira acima.
                </p>
              )}
            </div>
          </TabsContent>
          ...
          <TabsContent value="classes">
            {classes.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Sem turmas. Cria turmas a partir da aba Escolas.
              </p>
            ) : (
                    ...
                    <li key={c.id}>
                      ...
                      >
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                          {sc?.name} · Código:{" "}
                          <code className="rounded bg-muted px-1">{c.invite_code}</code>
                        </p>
                      </button>
                    </li>
                  );
          ...
          {selectedClass && (
            <TabsContent value="dash">
              <div className="card-chunky mb-4 rounded-2xl border-2 border-border bg-card p-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          reloadDashboard(selectedClass, e.target.value, daysFilter);
                        }}
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setDaysFilter(d);
                          reloadDashboard(selectedClass, subjectFilter, d);
                        }}
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          reloadAll(selectedClass, subjectFilter, daysFilter, e.target.value);
                        }}
                      >
                ...
                {/* Teacher alerts */}
                {alerts.filter((a) => !dismissedAlerts.has(`${a.studentId}:${a.kind}`)).length >
                  0 && (
                  <div className="mt-3 space-y-1">
                    {alerts
                      .filter((a) => !dismissedAlerts.has(`${a.studentId}:${a.kind}`))
                      .map((a) => {
                        const key = `${a.studentId}:${a.kind}`;
                        return (
                          <div
                            key={key}
                            className="flex items-center gap-2 rounded-lg border border-primary/30 bg-primary/10 px-2 py-1.5 text-xs"
                          >
                            <Bell className="h-4 w-4 text-primary" />
                            <Mascot id={a.mascot as MascotId} size="sm" />
                            <span className="font-display">{a.name}</span>
                            <span className="text-muted-foreground">
                              {a.kind === "accuracy_up" ? "📈" : "⏱️"} {a.detail}
                            </span>
                            <button
                    ...
                    <tbody>
                      {students.map((s) => (
                        <tr key={s.studentId} className="border-t border-border">
                          <td className="p-2">
                            <button
                              onClick={() => openStudentDetails(s.studentId)}
              ...
              <section className="card-chunky mt-4 rounded-2xl border-2 border-border bg-card p-4">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <h3 className="font-display text-lg">
                    Top 10 —{" "}
                    {subjectFilter ? (SUBJECT_LABELS[subjectFilter] ?? subjectFilter) : "Geral"}
                  </h3>
                </div>
                {ranking.length === 0 ? (
                  ...
                  <ol className="space-y-1">
                    {ranking.map((e, i) => (
                      ...
                      >
                        <span
                          className={`w-6 text-center font-display ${i < 3 ? "text-primary" : "text-muted-foreground"}`}
                        >
                          {i + 1}
                        </span>
                ...
                <p className="mt-2 text-[11px] text-muted-foreground">
                  A posição de cada aluno é mostrada também na tabela de relatório.
                </p>
              </section>
                        ...
                        <li key={s.studentId} className="flex items-center gap-2 py-2">
                          ...
                          >
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-muted-foreground">
                                {pos >= 0 ? `Posição #${pos + 1}` : "Sem ranking"} · {s.xp} XP ·{" "}
                                {s.accuracy}% precisão
                              </p>
                            </div>
                          </button>
                          <Button
                            ...
                            onClick={async () => {
                              ...
                              const r = await fnRemoveMember({
                                data: { classId: selectedClass.id, studentId: s.studentId },
                              });
                              if (r.ok) {
                          ...
                          >
                            <UserMinus className="mr-1 h-4 w-4 text-destructive" />
                            Remover
                          </Button>
                        </li>
                      );
                ...
                <section className="card-chunky mt-4 rounded-2xl border-2 border-primary/40 bg-card p-4">
                  <div className="mb-3 flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {openStudent.grade}.º ano · 🔥 {openStudent.streak} · {openStudent.xp} XP
                        totais
                      </p>
                    </div>
                    <Button
                      ...
                      {Object.entries(openStudent.bySubject).map(([sid, v]) => (
                        <div key={sid} className="rounded-lg bg-muted/40 p-2 text-xs">
                          ...
                          <p className="text-muted-foreground">
                            {v.sessions} sessões · {v.accuracy}% · {v.minutes}m
                          </p>
                        </div>
                      ))}
                    </div>
                      ...
                      {openStudent.recent.map((r) => (
                        <li key={r.id} className="flex items-center gap-2 py-1.5">
                          ...
                          <span className="flex-1 font-display">
                            {SUBJECT_LABELS[r.subject_id] ?? r.subject_id}{" "}
                            <span className="text-muted-foreground">· {r.lesson_id}</span>
                          </span>
                          <span>
                            {r.correct}/{r.total}
                          </span>
    ...
    >
      <div className="flex-1">
        ...
        <Input
          ...
          placeholder="Ex.: EB Sol Nascente"
        />
      </div>
...
function NewClass({
  ...
  schoolId: string;
  onCreate: (data: { schoolId: string; name: string; grade: number }) => void;
}) {
  const [name, setName] = useState("");
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        if (name.trim()) {
          onCreate({ schoolId, name: name.trim(), grade });
          setName("");
        }
      }}
