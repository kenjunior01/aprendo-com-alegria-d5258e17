...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Entra na tua conta para guardar o teu progresso em todos os dispositivos.",
      },
    ...
    links: [{ rel: "canonical", href: "https://kidoz.online/auth" }],
  }),
  component: AuthPage,
...
function AuthPage() {
  ...
  const handleOAuth = async (provider: "google" | "apple") => {
    ...
    try {
      ...
      if (result.error) {
        setError(
          `Não foi possível entrar com ${provider === "google" ? "Google" : "Apple"}. Tenta outra vez.`,
        );
        return;
      }
...
function Field({ label, icon, value, onChange, error, id, ...rest }: FieldProps) {
  ...
  return (
    <label htmlFor={fieldId} className="block">
      ...
      <div className="flex items-center gap-2 rounded-2xl border-2 border-border bg-card px-4 py-3 focus-within:border-primary has-[:user-invalid]:border-destructive">
        {icon && (
