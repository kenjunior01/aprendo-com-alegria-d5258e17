...
export const Route = createFileRoute("/licao/$subjectId/$lessonId")({
  validateSearch: (search: Record<string, unknown>): { challenge?: string } => {
    const out: { challenge?: string } = {};
    if (typeof search.challenge === "string") out.challenge = search.challenge;
    return out;
  },
  head: ({ params }) => {
  ...
  const onNext = () => {
    if (qIndex + 1 >= total) {
      ...
      if (challengeId) {
        ...
        void fnSubmitChallenge({ data: { challengeId, score } }).catch((e) =>
          console.error("submitChallengeScore", e),
        );
      }
    ...
    return (
      <main className="bg-paper flex min-h-[100dvh] flex-col items-center justify-center px-6 text-center">
        ...
        <div className="mt-6 flex w-full max-w-[24rem] flex-col gap-3 sm:flex-row">
          ...
          <ChunkyButton onClick={() => window.location.reload()} className="flex-1">
            Tentar outra vez
          </ChunkyButton>
        </div>
      </main>
