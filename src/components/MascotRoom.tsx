...
const StickerAlbum = lazy(() =>
  import("./StickerAlbum").then((m) => ({ default: m.StickerAlbum })),
);
const StickerPackOpening = lazy(() =>
  import("./StickerPackOpening").then((m) => ({ default: m.StickerPackOpening })),
);
...
export function MascotRoom({ profile }: Props) {
  ...
  const getBackground = () => {
    switch (room) {
      ...
      case "bedroom":
        return "bg-gradient-to-b from-indigo-200 to-purple-300";
      case "classroom":
        return region === "MZ"
          ? "bg-gradient-to-b from-emerald-100 to-yellow-50"
          : "bg-gradient-to-b from-blue-100 to-emerald-100";
      case "talk":
        return "bg-gradient-to-b from-indigo-100 to-sky-200";
      case "games":
        return "bg-gradient-to-b from-pink-100 to-rose-200";
      default:
        return "bg-gradient-to-b from-sky-200 to-white";
    }
  };
  ...
  const stats = [
    {
      ...
      color: profile.hunger < 25 ? "bg-red-500" : "bg-orange-400",
      label: profile.hunger < 25 ? "Fome baixa" : "Fome",
    ...
    },
    {
      ...
      color: profile.energy < 25 ? "bg-red-500" : "bg-yellow-400",
      label: profile.energy < 25 ? "Energia baixa" : "Energia",
    ...
    },
    {
      ...
      icon: Gamepad2,
      value: profile.fun,
      color: profile.fun < 25 ? "bg-red-500" : "bg-pink-400",
      label: profile.fun < 25 ? "Diversão baixa" : "Diversão",
      room: "living" as RoomType,
    },
    {
      id: "knowledge",
      icon: GraduationCap,
      value: profile.knowledge,
      color: profile.knowledge < 30 ? "bg-red-600 animate-pulse" : "bg-blue-400",
      label: profile.knowledge < 30 ? "Conhecimento baixo" : "Conhecimento",
      room: "classroom" as RoomType,
    },
    {
      id: "talk",
      icon: Mic,
  ...
  return (
    <div
      ...
      className={cn(
        "relative flex h-full w-full flex-col overflow-hidden transition-colors duration-700 touch-none select-none",
        getBackground(),
      )}
    >
      ...
      <div className="relative z-10 flex justify-between p-4 px-6 pt-6">
        <div className="flex gap-2">
          ...
          >
            ...
            <span className="font-display text-base font-black text-orange-700">
              {profile.streak}
            </span>
          </motion.div>
        </div>
          ...
          <div className="w-full max-w-[28rem] px-6 h-full overflow-y-auto pt-10 pb-20 scrollbar-none">
            {!selectedGame ? (
              ...
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="rounded-3xl border-4 border-white bg-white/80 p-6 shadow-xl"
              >
                ...
                <div className="mt-6 flex justify-center">
                  <button
                    onClick={() => {
                      ...
                      playFun();
                      updateProfile({
                        fun: Math.min(100, profile.fun + 20),
            ...
            <AnimatePresence mode="wait">
              {hasGift && (
                ...
                >
                  ...
                  <span className="font-display text-[10px] font-black uppercase text-amber-700">
                    {t("Presente!")}
                  </span>
                </motion.button>
              )}
              {room === "kitchen" && (
                <motion.span
                  ...
                  key="kitchen-icon"
                  className="absolute left-10 top-1/2 text-6xl"
                >
                  🥣
                </motion.span>
              )}
              {room === "bedroom" && (
                <motion.span
                  ...
                  exit={{ opacity: 0, scale: 0 }}
                  key="bedroom-icon"
                  className="absolute right-10 top-1/3 text-6xl"
                >
                  🛌
                </motion.span>
              )}
          ...
          {stats.map((stat) => (
            <div key={stat.id} className="flex flex-col items-center gap-2">
              <motion.button
                ...
                className={cn(
                  "relative flex h-16 w-16 items-center justify-center rounded-3xl border-4 border-white shadow-xl transition-all",
                  room === stat.room ? stat.color : "bg-white/90",
                )}
              >
                <stat.icon
                  className={cn("h-7 w-7", room === stat.room ? "text-white" : "text-slate-600")}
                />
              ...
              {stat.id !== "talk" && (
                <div
                  className="h-2 w-12 overflow-hidden rounded-full bg-slate-200/50"
                  role="progressbar"
      ...
      <AnimatePresence>
        ...
        {newSticker && (
          <StickerPackOpening sticker={newSticker} onClose={() => setNewSticker(null)} />
        )}
      </AnimatePresence>
    </div>
  );
